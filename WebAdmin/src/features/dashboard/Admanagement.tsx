...
import { Search, Plus, Pencil, Trash2, X, Users, LayoutList, Upload, Video } from "lucide-react";
...
const AdModal: React.FC<{ mode: "add" | "edit"; ad: Partial<Ad>; onClose: () => void; onSave: (data: any) => void }> = ({ mode, ad, onClose, onSave }) => {
  ...
  const handleSave = () => {
    if (!form.title || !form.advertiser || !form.imageUrl || !form.linkUrl) return;
    onSave({ ...form, adDuration: Number(form.adDuration) || 30 });
    setSaved(true);
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-5 space-y-4 overflow-y-auto flex-1">
          ...
          <div>
            ...
            {videoUrl ? (
              <div className="relative rounded-xl overflow-hidden bg-black">
                ...
                <button onClick={() => { setVideoUrl(""); setVideoName(""); }}
                  className="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/60 flex items-center justify-center text-white hover:bg-black/80 transition-colors"><X size={12} /></button>
                <div className="absolute bottom-0 left-0 right-0 px-3 py-2 bg-gradient-to-t from-black/70 to-transparent">
                  <p className="text-white text-xs truncate">{videoName}</p>
                </div>
              </div>
            ) : (
              <div onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
                ...
                <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center"><Video size={18} className="text-gray-400" /></div>
                <div className="text-center">
                  <p className="text-sm font-medium text-gray-600"><span className="text-blue-500">คลิกเพื่ออัปโหลด</span> หรือลากไฟล์มาวาง</p>
                  <p className="text-xs text-gray-400 mt-0.5">MP4, MOV, AVI, WebM</p>
                </div>
                <div className="flex items-center gap-1.5 mt-1"><Upload size={12} className="text-gray-400" /><span className="text-xs text-gray-400">รองรับทุกรูปแบบวิดีโอ</span></div>
              </div>
            )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className={labelCls}>ชื่อโฆษณา *</label><input type="text" placeholder="ชื่อโฆษณา" value={form.title || ""} onChange={(e) => setForm({ ...form, title: e.target.value })} className={inputCls} /></div>
            <div><label className={labelCls}>ผู้ลงโฆษณา *</label><input type="text" placeholder="ชื่อบริษัท" value={form.advertiser || ""} onChange={(e) => setForm({ ...form, advertiser: e.target.value })} className={inputCls} /></div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={form.adType || "SPLASH"} onChange={(e) => setForm({ ...form, adType: e.target.value })} className={inputCls}>
            ...
            <div><label className={labelCls}>ความยาว (วินาที)</label><input type="number" min={0} value={form.adDuration ?? 30} onChange={(e) => setForm({ ...form, adDuration: Number(e.target.value) })} className={inputCls} /></div>
          </div>

          {/* Image URL + Link URL */}
          <div><label className={labelCls}>Image URL *</label><input type="text" placeholder="https://..." value={form.imageUrl || ""} onChange={(e) => setForm({ ...form, imageUrl: e.target.value })} className={inputCls} /></div>
          <div><label className={labelCls}>Link URL *</label><input type="text" placeholder="https://..." value={form.linkUrl || ""} onChange={(e) => setForm({ ...form, linkUrl: e.target.value })} className={inputCls} /></div>
          ...
          <div>
            ...
            <select value={form.isActive ? "true" : "false"} onChange={(e) => setForm({ ...form, isActive: e.target.value === "true" })} className={inputCls}>
...
const AdManagementPage: React.FC = () => {
  ...
  return (
    <div className="flex min-h-screen">
            ...
            <div className="bg-gray-800 rounded-xl overflow-hidden">
              <div className="px-5 py-4">
                <div className="relative">
                  ...
                  <input type="text" placeholder="ค้นหาโฆษณา..." value={search} onChange={(e) => setSearch(e.target.value)}
                      ...
                      <tr key={ad.id} className="hover:bg-gray-700/40 transition-colors">
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-md bg-gray-700 flex items-center justify-center flex-shrink-0"><Video size={12} className="text-gray-500" /></div>
                            <div>
                        ...
                        <td className="px-4 py-3.5"><span className="px-2 py-0.5 bg-gray-700 text-gray-300 rounded-md text-xs">{AD_TYPE_LABEL[ad.adType] || ad.adType}</span></td>
                        ...
                        <td className="px-4 py-3.5 text-right text-white text-xs font-medium">{ad.impressions.toLocaleString()} ครั้ง</td>
                        <td className="px-4 py-3.5 text-center">
                          <div className="flex items-center justify-center gap-1">
                            <button onClick={() => toggleAds(ad.id)} className={`px-2 py-1 rounded text-xs transition-colors ${ad.isActive ? "text-yellow-400 hover:bg-yellow-500/10" : "text-green-400 hover:bg-green-500/10"}`}>
                              {ad.isActive ? "ปิด" : "เปิด"}
                            </button>
                            <button onClick={() => setEditAd(ad)} className="p-1.5 rounded-lg text-gray-400 hover:text-white hover:bg-gray-600 transition-colors"><Pencil size={14} /></button>
                            <button onClick={() => setDeleteAd(ad)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-400 hover:bg-red-500/10 transition-colors"><Trash2 size={14} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
      ...
      {addModal && <AdModal mode="add" ad={{ adType: "SPLASH", adDuration: 30, isActive: true }} onClose={() => setAddModal(false)} onSave={createAds} />}
      {editAd && <AdModal mode="edit" ad={editAd} onClose={() => setEditAd(null)} onSave={(data) => { updateAds(editAd.id, data); setEditAd(null); }} />}
      {deleteAd && <DeleteModal ad={deleteAd} onClose={() => setDeleteAd(null)} onConfirm={handleDelete} />}
    </div>
  );
};
